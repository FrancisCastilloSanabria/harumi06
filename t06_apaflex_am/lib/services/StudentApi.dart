...
Future<Student> saveStudent(Map<String, dynamic> studentData) async {
  final response = await http.post(
    ...
    headers: {
      'Content-Type': 'application/json', // Configura el tipo de contenido a JSON
    },
  );
  if (response.statusCode == 200) {
    return Student.fromJson(jsonDecode(response.body));
  } else {
    throw Exception('Failed to save student: ${response.statusCode} - ${response.body}');
  }
}
...
final response = await http.delete(url);
  ...
  final api = StudentApi("http://192.168.41.18:8085/oraclecloud/v1/students");